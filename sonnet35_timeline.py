...
def load() -> pd.DataFrame:
    ...
    df["sonnet35_match"] = text.apply(
        lambda t: any(re.search(k, t) for k in SONNET35_KW)
    )
    ...
    log.info(
        f"In-scope posts: {len(scope)} / {len(df)}  "
...
def _phase_legend():
    handles = [
        mpatches.Patch(
            color=PHASE_COLORS[p], alpha=0.7, label=f"{p}  (day {lo:+d} → {hi:+d})"
        )
        for p, (lo, hi) in PHASES.items()
    ]
    handles.append(
        plt.Line2D(
            [0], [0], color="#E74C3C", lw=1.8, ls="--", label="Release day (Jun 20)"
        )
    )
    return handles
...
def chart_daily_volume(daily: pd.DataFrame):
    ...
    ax.plot(
        daily["days"], daily["posts"], color="#2C3E50", lw=2, marker="o", ms=4, zorder=5
    )
    ax.fill_between(daily["days"], daily["posts"], alpha=0.12, color="#2C3E50")
    ax.set_title(
        f"{RELEASE_LABEL} — Daily Post Volume\n"
...
def chart_daily_engagement(daily: pd.DataFrame):
    ...
    ax.set_title(
        f"{RELEASE_LABEL} — Daily Total Engagement\n"
...
def chart_category_mix(cat_tbl: pd.DataFrame):
    ...
    ax.set_title(
        f"{RELEASE_LABEL} — Content Category Mix by Phase\n"
...
def chart_author_mix(auth_tbl: pd.DataFrame):
    ...
    ax.set_title(
        f"{RELEASE_LABEL} — Who Drove Each Phase?\n"
...
def chart_top_posts(top: pd.DataFrame):
    ...
    ax.set_title(
        f"{RELEASE_LABEL} — Top Posts by Engagement  (days −3 to +14)\n"
...
def print_summary(phase_tbl, daily, top):
    ...
    for _, row in top.head(5).iterrows():
        ...
        print(
            f"  [{row['phase']}] {row['source_platform'].upper()} | "
