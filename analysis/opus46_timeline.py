...
Answers: "How is Claude attention CREATED, AMPLIFIED, and SUSTAINED
          after the Claude Opus 4.6 release (February 2026)?"

Usage:
    PYTHONPATH=. .venv/bin/python opus46_timeline.py

Reads:  data/final/dataset.csv  (existing pipeline output)
Writes: outputs/charts/opus46_*.png
        outputs/tables/opus46_*.csv
"""
...
RELEASE_DATE  = pd.Timestamp("2026-02-05")   # Anthropic published "Introducing Claude Opus 4.6"
RELEASE_LABEL = "Claude Opus 4.6"
BENCHMARKS    = ["SWE-bench 80.8%", "ARC-AGI-2 68.8%", "HumanEval 97%+", "200K ctx"]
...
# Keywords that mark a post as Opus 4.6 relevant
OPUS_KW = [
    r"opus\s*4\.6", r"claude\s+opus\s+4", r"claude\s+4\.6",
    r"swe.bench", r"arc.agi.2", r"80\.8", r"68\.8",
    r"most\s+capable.*model", r"model.*most\s+capable",
    r"introducing\s+claude\s+opus", r"claude.*1m\s+context",
    r"opus.*agentic", r"greatest\s+ai\s+coding",
    r"claude.*conscious", r"did\s+claude\s+become",
    r"claude\s+cowork.*opus", r"opus.*cowork",
    r"claude\s+4\.5",    # predecessor — pre-launch chatter
    r"claude\s+4\b",     # generic Claude 4 conversation
    r"new\s+claude\s+model", r"anthropic.*new\s+model",
]
...
def load() -> pd.DataFrame:
    ...
    df["opus_match"] = text.apply(lambda t: any(re.search(k, t) for k in OPUS_KW))

    # In-scope = keyword match OR within window
    df["in_scope"] = df["opus_match"] | df["days"].between(-30, 60)

    scope = df[df["in_scope"]].copy()
    log.info(f"In-scope posts: {len(scope)} / {len(df)}  "
             f"(keyword hits: {df['opus_match'].sum()}, window: {df['days'].between(-30,60).sum()})")
    return scope
...
def _phase_legend():
    ...
    handles.append(plt.Line2D([0],[0], color="#E74C3C", lw=1.8,
                               ls="--", label="Release day (Feb 5)"))
    return handles

def _save(fig, name):
    p = OUT_CHARTS / f"opus46_{name}"
...
def chart_daily_volume(daily: pd.DataFrame):
    ...
    ax.set_xlabel("Days relative to release  (0 = Feb 5 2026)", fontsize=11)
...
def print_summary(phase_tbl, daily, top):
    ...
    print(f"\n─── Outputs ───────────────────────────────────────────────")
    print(f"  Charts : outputs/charts/opus46_*.png  (6 charts)")
    print(f"  Tables : outputs/tables/opus46_*.csv  (5 tables)")
    print(f"{'='*62}\n")
...
def main():
    ...
    # Save tables
    phase_tbl.to_csv(OUT_TABLES / "opus46_phase_summary.csv",    index=False)
    daily.to_csv(    OUT_TABLES / "opus46_daily_volume.csv",      index=False)
    cat_tbl.to_csv(  OUT_TABLES / "opus46_category_by_phase.csv", index=False)
    auth_tbl.to_csv( OUT_TABLES / "opus46_author_by_phase.csv",   index=False)
    top.to_csv(      OUT_TABLES / "opus46_top_posts.csv",         index=False)
